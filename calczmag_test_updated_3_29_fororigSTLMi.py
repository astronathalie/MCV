...
class SDSS_catalog:
    ...
    def get_catalog(self, wcs, filter, mag_range=(12, 16), num_sources=200, mag_limit=20):
        ...
        i = 0
        while job is not None and len(job) < 50 and mag_max < mag_limit:
            i  = i + 1
            mag_max = mag_max + 1
            sdss_query = f"""
            SELECT top {num_sources} p.objID, p.ra, p.dec, p.{filter}, p.clean
    ...
    def get_pan_starrs_catalog(self, wcs, filter, mag_range=(12, 16), num_sources=300, mag_limit=20):
        ...
        while top_result is not None and len(top_result) < 20 and mag_max < mag_limit:
            mag_max = mag_max + 1
            i = i + 1
            v = Vizier(columns=['objID', 'RAJ2000', 'DEJ2000', mag_column, 'gmag', 'rmag'], 
...
class ImageProcessor:
    def __init__(self, image):
        ...
        if not self.wcs or not self.wcs.has_celestial:
            ...
            #stop the program
            exit()
        self.sdss_catalog = SDSS_catalog()
    ...
    def sdss_check(self, filter=None):
        ...
        catalog = self.sdss_catalog.get_catalog(self.wcs, filter=filter)
        sdss_sources = []
        color = []
    ...
    def zmag_calc(self):
        ...
        filter = self.header.get('FILTER').lower()
        sdss_data, color = self.sdss_check(filter=filter)

        try: # From Tonry et al. 2012 https://iopscience.iop.org/article/10.1088/0004-637X/750/2/99#apj425122s5
